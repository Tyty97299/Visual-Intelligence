...
import { X, Send, Sparkles, ChevronRight, Globe, Cpu, ArrowLeft } from 'lucide-react';
...
export const PhotoDetail: React.FC<PhotoDetailProps> = ({ photo, onClose, onAsk }) => {
  ...
  // Control view state: 'preview' (bubbles/card) or 'chat' (conversation)
  const [showChat, setShowChat] = useState(false);
  const scrollRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    // If we just entered chat mode and there is history, scroll to bottom
    if (showChat && scrollRef.current) {
      scrollRef.current.scrollTop = scrollRef.current.scrollHeight;
    }
  }, [showChat, photo.chatHistory]);
  ...
  const handleNavigation = () => {
    if (showChat) {
        // Go back to Preview Mode (Bubbles + Smart Card)
        setShowChat(false);
    } else {
        // Close the Detail view completely
        onClose();
    }
  };

  const hasChatHistory = photo.chatHistory.length > 0;
    ...
    <div className="fixed inset-0 z-50 bg-black flex flex-col h-[100dvh] font-sans">
      ...
      <div className="absolute top-0 left-0 w-full z-[60] flex justify-between items-center p-4 pointer-events-auto">
        <button 
            ...
            aria-label={showChat ? "Back to Preview" : "Close"}
        >
          {showChat ? (
      ...
      {/* Main Image Area */}
      <div className={`absolute inset-0 z-0 transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] ${showChat ? 'h-[35%] opacity-40' : 'h-full opacity-100'}`}>
        <img 
      ...
      <div className="absolute inset-0 z-10 flex flex-col justify-end pointer-events-none">
        ...
        {/* Chat Area (Only visible when showChat is true) */}
        {showChat && (
             <div 
                ref={scrollRef}
        ...
        {/* Smart Card (Preview View) - Shown when NOT in chat mode */}
        {!showChat && !photo.isLoadingSuggestions && photo.smartCard && (
             <div className="w-full px-4 mb-4 pointer-events-auto z-20 animate-fade-in-up">
                 <div className="max-w-xl mx-auto bg-zinc-950/90 backdrop-blur-xl border border-zinc-800 rounded-2xl overflow-hidden shadow-2xl ring-1 ring-white/10">
        ...
        {/* Suggestions (Preview View) - Shown when NOT in chat mode */}
        {!showChat && (
             <div className="w-full px-4 pb-6 pointer-events-auto z-20 animate-fade-in-up">
        ...
        <div className={`w-full bg-black border-t border-zinc-800 p-4 pt-3 pb-6 pointer-events-auto z-30 ${!showChat && 'rounded-t-3xl border-t-cyan-500/20 shadow-[0_-5px_30px_rgba(0,0,0,0.8)]'}`}>
